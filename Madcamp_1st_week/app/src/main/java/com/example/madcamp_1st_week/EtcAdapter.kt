...
class EtcAdapter (private val context: Context): RecyclerView.Adapter<EtcAdapter.PagerViewHolder>(){

    var etcs = mutableListOf<FriendItem>()
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): EtcAdapter.PagerViewHolder {
//        val view = LayoutInflater.from(context).inflate(R.layout.item_recyclerview,parent,false)
        val view = LayoutInflater.from(context).inflate(R.layout.contact_item,parent,false)
        return PagerViewHolder(view)
    }

    override fun getItemCount(): Int = etcs.size
    override fun onBindViewHolder(holder: PagerViewHolder, position: Int) {
        holder.bind(etcs[position])
    }
